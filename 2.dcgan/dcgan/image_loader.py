...
class ImageLoader(object):
    def __init__(self, path, shuffle=True, resize=None, center_crop=None, color='auto', norm_range=[-1, 1]):
        self._PIXEL_MAX = 255
    ...
    def _init_img_list(self, path, shuffle):
    ...
    def _read_image(self, path):
        if self.image_shape[2] == 1:
            image = scipy.misc.imread(path, flatten=True).astype(np.float)
        else:
            image = scipy.misc.imread(path).astype(np.float)

        if self.center_crop is not None:
            image = _crop_center(image, self.center_crop)

        if self.resize is not None:
            image = scipy.misc.imresize(image, resize)

        if self.norm_range is not None:
            image = (image / (_PIXEL_MAX / (self.norm_range[1] - self.norm_range[0]))) + self.norm_range[0]
